...
export const db = {
  ...
  async updateComputer(id: string, data: any) {
    if (isPostgresSync) {
        // Simple update logic for demo/diploma purposes
        // In real app, build dynamic SQL or use ORM
        const fields = Object.keys(data).filter(k => k !== 'id' && k !== 'history' && k !== 'software');
  ...
  async upsertComputer(data: any) {
    ...
    if (existing.rows.length > 0) {
      ...
      const fields = ['id', 'name', 'model', 'manufacturer', 'serial_number', 'status', 'assigned_to', 'location', 'cpu', 'cpu_cores', 'ram', 'storage', 'gpu', 'motherboard', 'os', 'last_ip', 'vnc_active', 'notes', 'last_seen'];
      const values = [
        id,
